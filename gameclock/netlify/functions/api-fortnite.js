...
const { fetchFortniteStats } = require('./lib/fortnite');
...
exports.handler = async (event) => {
  ...
  const cacheStore = getBlobStore('fortnite-cache');
  ...
  try {
    const stats = await fetchFortniteStats(links.fortniteName, process.env.FORTNITE_API_KEY);
    ...
    if (cached) {
      // Serve stale data rather than nothing if the live fetch fails.
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
